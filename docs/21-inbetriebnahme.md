...
**Auch der Caddyfile steht im Repository:** [`ops/caddy/Caddyfile`](../ops/caddy/Caddyfile). Er trägt die Ratenbegrenzung aus Dokument 17 §4 und die Sicherheitskopfzeilen. Die Namen darin sind seit der Umbenennung echt: `app.staygrid.cloud` für Menschen, `api.staygrid.cloud` für Maschinen, `admin@staygrid.cloud` als Adresse für Let's Encrypt. **Daneben steht weiterhin `pms.wattnauftritt.de`**, der Name, unter dem die Rezeption vor dem Umstieg gearbeitet hat: beide liefern dasselbe aus, bis niemand mehr den alten benutzt. Ihn beim Umstieg einfach zu ersetzen hieße, dass jemand mitten im Check-in einen Zertifikatsfehler sieht. **Beide Namen müssen im DNS stehen, bevor Caddy sie zum ersten Mal sieht** — sonst scheitert die Zertifikatsanfrage, und zwar mit einem Fehlerbild, das nach einem kaputten Dienst aussieht. Liegt die Zone hinter einem Proxy wie Cloudflare, gehören genau diese beiden Einträge auf **DNS-only**: ein Proxy beendet TLS selbst, und damit kommt Caddys TLS-ALPN-Prüfung nie an. Dazu `PUBLIC_APP_URL=https://app.staygrid.cloud` in der Umgebungsdatei; steht sie falsch, zeigen die Links in Einladung und Kennwortrücksetzung ins Leere.
...
**Was zurückgerollt werden kann**, entscheidet die Platte: der Agent trägt bei jedem Tick ein, welche Stände mit `.fertig` unter `releases/` liegen und worauf `current` zeigt (Tabelle `release`, Migration 0041). Das Panel bietet genau diese an, ohne den laufenden. Ein Stand, der von Hand ausgerollt wurde oder dessen Lauf nach dem Umschalten scheiterte, ist damit genauso ein Ziel wie ein geglückter — er liegt ja da. Neben jedem steht, wann er gebaut wurde: die Änderungszeit von `.fertig` (Migration 0042), nicht die des Commits und nicht die des Ausrollens.
