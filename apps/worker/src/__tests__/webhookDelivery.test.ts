...
import { webhookSignature, WEBHOOK_HEADERS, WEBHOOK_MAX_ATTEMPTS } from '@hotelpms/domain'
...
describe('Zustellung ausgehender Ereignisse', () => {
  ...
  it('bindet den Zeitstempel in die Signatur ein', async () => {
    ...
    try {
      ...
      await deliverWebhooks(app, ctx, fx.propertyId)
  ...
  it('wiederholt mit wachsendem Abstand und legt danach das Abonnement still', async () => {
    ...
    try {
      ...
      const erste = await deliverWebhooks(app, ctx, fx.propertyId)
      ...
      const zweite = await deliverWebhooks(app, ctx, fx.propertyId)
      ...
      await faelligStellen()
      const dritte = await deliverWebhooks(app, ctx, fx.propertyId)
      ...
      expect(a.rows[0]!.disabled_reason).toContain('HTTP 500')
  ...
  it('stellt an ein stillgelegtes Abonnement nichts mehr zu', async () => {
    ...
    try {
      ...
      expect(await deliverWebhooks(app, ctx, fx.propertyId))
  ...
  it('behandelt einen unerreichbaren Empfaenger wie eine Fehlerantwort', async () => {
    ...
    expect(z.last_error).not.toBeNull()
  })
