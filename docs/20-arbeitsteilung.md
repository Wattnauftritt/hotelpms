...
| B5 | Rechnungsliste | fertig | #30, #39 | Neuer Endpunkt `GET .../invoices`. Der Zahlungsstand kam nach, sobald die Zuordnung einen Schreiber hatte (Abschnitt 6) |
...
| B8 | Anzahlung | fertig | #33 | Eingeklapptes Feld am Folio. Neuer Endpunkt `GET /v1/folios/:ref/prepayments`; die Verrechnung steht an der Anzahlung, sobald die Schlussrechnung sie zieht |
| B9 | Pay-by-Link | fertig | #33 | Im selben Feld. Die Adresse wird **einmal** gezeigt und nicht gespeichert; ohne Zahlungsdienstleister sagt die Maske das, statt eine 503 zu zeigen |
| B10 | Was der Channel Manager sieht | fertig | #33 | Unter dem Raster. Neuer Endpunkt `GET .../channel-view` mit **demselben SQL** wie ARI; ein Klick aufs Datum zeigt die Rohantwort dieses Tages |
...
| B | (Rahmen) | **Der Geldformatierer wird jetzt gemerkt.** `formatMoney` baute bei jedem Aufruf ein `Intl.NumberFormat`; im Browser gemessen sind das auf 16 000 Formatierungen 512 ms gegen 12 ms. Er liegt samt `geldFormatierer` in `lib/i18n/index.ts`, mit Sprache **und** Währung als Schlüssel; der eigene in `lib/preisraster.ts` entfällt. Dabei mitbehoben: `weekdayShort` hatte denselben Fehler und wird **je Tagesspalte** in drei Rastern aufgerufen | erledigt |
...
| B | (alle) | **`settlement.invoice_id` hat jetzt einen Schreiber.** Zugeordnet wird beim Festschreiben (was schon vermerkt war, bis zum Zahlbetrag) und beim Vermerken (an die älteste offene Rechnung des Folios). Damit steht der vorausgezahlte Betrag als BT-113 auf dem Beleg, und die Rechnungsliste führt „offen" und „bezahlt". Ausgenommen sind Erstattungen und Vermerke, aus denen eine Anzahlungsrechnung geworden ist — die stünden sonst zweimal. **Zahlungen von vor dieser Änderung** bleiben ohne Rechnungsnummer: `NULL` → Wert ist der einzige erlaubte Übergang, und für alte Zeilen darf ihn niemand raten | erledigt |
| B | (Rahmen) | **Die Rechte stehen jetzt am `ScreenContext`.** Ein Bildschirm kam vorher nicht an sie heran und las denselben Zwischenspeicher (`['me']`) noch einmal — was ging, aber eine Umgehung war. `permission` am Eintrag entscheidet weiterhin, **ob** ein Bildschirm erscheint; `permissions` im Kontext entscheidet, **was darin** erscheint. `lib/queries/rechte.ts` ist weg, ein Test hält die Umgehung fern | erledigt |
| B | (alle) | **`sum()` über eine `bigint`-Spalte liefert `numeric`, und `numeric` kommt als Zeichenkette an.** Der Vorschlag, `numeric` in den Typenparser zu nehmen, ist nach Prüfung **verworfen**: er nähme die Sicherung gegen stilles Runden heraus, die es für eine echte `numeric`-Spalte braucht — es gibt im Schema heute keine, und genau deshalb würde die erste lautlos falsch gelesen. Es bleibt bei der Regel: wer eine Summe zurückgibt, castet sie. Ein Test in `packages/db` hält das Verhalten fest und sieht alle Routen auf ungecastete Summen durch; die Falle steht in den Fallstricken von `docs/16` | erledigt |
...
| B | (gefunden bei A1) | `Folio.tsx` zeigte den Hinweistext von `GET .../payment-methods` unübersetzt. Inzwischen behoben, und zwar im Rahmen: die Antwort trägt den Schlüssel neben dem Satz (`hinweisKey`), und `apiText` in `lib/meldungen.ts` gibt ihn in der Sprache des Personals aus. Nachgesehen: der Satz wechselt beim Umschalten auf Englisch mit | erledigt |
